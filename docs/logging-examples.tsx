...
import { useLogger, useApiLogger, usePerformanceLogger, useFormLogger } from '@/hooks/use-logger';
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        setProfile(data);
        log.info('User profile loaded successfully', { userId, profileId: data.id });
      } catch (error) {
...
export function DataFetcher() {
  ...
  const fetchData = async () => {
    ...
    await measureAsyncPerformance('fetch_dashboard_data', async () => {
      // Log API call with automatic request/response tracking
      const result = await logApiCall('GET', '/api/dashboard/data', async () => {
...
export function InteractiveWidget() {
  ...
  const handleFeatureUsage = (feature: string, metadata?: Record<string, any>) => {
  ...
  return (
    <div>
      ...
      <button onClick={() => handleFeatureUsage('advanced_feature', { level: 'expert' })}>
        Advanced Feature
      </button>
    </div>
  );
  ...
  return (
    <ErrorBoundary
      onError={(error, errorInfo) => {
        ...
        logger.error('Feature-specific error occurred', error, {
...
export function PerformanceCriticalComponent() {
  ...
  const expensiveCalculation = () => {
    return measurePerformance('expensive_calculation', () => {
