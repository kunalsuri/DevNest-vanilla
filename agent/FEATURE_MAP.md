...
> **`status` and `safety` are owned by [`INDEX.yaml`](INDEX.yaml)** (the single machine-readable
> source). They are intentionally NOT repeated in this table — if you need a feature's safety
> level or status, read `INDEX.yaml`. This file owns the _descriptive_ detail below.

| Feature ID | Feature Name           | Description                                                                            | Owner         | Tech Debt                 | Notes                             |
| ---------- | ---------------------- | -------------------------------------------------------------------------------------- | ------------- | ------------------------- | --------------------------------- |
| F-01       | Authentication (JWT)   | Complete JWT auth system with access/refresh tokens, CSRF, session management, lockout | Auth          | Email delivery not impl   | See authentication.md             |
| F-02       | User Profile           | User profile CRUD with picture upload, professional info                               | Profile       | None                      | Fully tested                      |
| F-03       | Account Preferences    | Per-user preferences: theme, notifications, 2FA toggle (UI only), auto-logout          | Profile       | 2FA backend missing       | 2FA UI exists but no server logic |
| F-04       | Admin User Management  | Admin-only user CRUD: list, view, role change, delete                                  | Admin         | None                      | Protected by requireAdmin         |
| F-05       | Feature Flags          | Admin-controlled feature flags with per-user targeting                                 | Admin         | None                      | JSON-based storage                |
| F-06       | Notifications          | In-app notification system with CRUD endpoints                                         | Notifications | UI incomplete             | API complete, client partial      |
| F-07       | Subscriptions          | Subscription plan management                                                           | Subscriptions | Business logic incomplete | API scaffolded                    |
| F-08       | Dashboard              | Main authenticated dashboard with stats widgets                                        | Dashboard     | Data binding partial      | UI scaffold exists                |
| F-09       | Workspaces             | Multi-workspace support                                                                | Workspaces    | No backend API            | Client UI only                    |
| F-10       | Observability (Client) | Client-side error capture, logging, metrics, tracing                                   | Observability | None                      | Sentry integrated                 |
| F-11       | Observability (Server) | Server logging, Sentry, distributed tracing, audit logs                                | Observability | None                      | Winston + Sentry                  |
| F-12       | Storage Layer          | FileStorage implementation with JSON persistence                                       | Storage       | Scale bottleneck          | Migration path to PG available    |
| F-13       | App Shell / Navigation | Sidebar, navbar, layouts, routing, theme provider                                      | UI            | None                      | Responsive design                 |
| F-14       | Health Checks          | Liveness and readiness endpoints                                                       | Health        | None                      | Used by Docker                    |
| F-15       | API Documentation      | OpenAPI 3.0 docs via Swagger                                                           | Docs          | None                      | Auto-generated from JSDoc         |
...
- [agent/features/INDEX.md](features/INDEX.md) — High-level feature list
- [agent/features/authentication.md](features/authentication.md) — Detailed auth feature doc
- [agent/architecture/MODULE_MAP.md](architecture/MODULE_MAP.md) — File-to-module mapping
- [agent/architecture/OVERVIEW.md](architecture/OVERVIEW.md) — System architecture
- [agent/AGENT_GUIDE.md](AGENT_GUIDE.md) — Agent operating instructions
- [agent/CHANGE_POLICY.md](CHANGE_POLICY.md) — What agents can modify
...
### F-16: Chart Recharts Type Fix

**Description:** Aligns the shadcn `chart.tsx` wrapper with the Recharts **v3** type
surface (the project ships `recharts@^3.8.1`). v3 omits context-injected props
(`payload`/`label`) from `TooltipProps` and `payload` from `LegendProps`; the wrapper now
types them via Recharts' exported `TooltipContentProps`/`LegendPayload`. Type-only change —
no runtime behavior difference. Resolves the ~9 `tsc` errors that previously made
`npm run check` (and CI's `lint-and-typecheck` job) fail.

**Owner:** UI / shared components | **Status & Safety:** authoritative in `INDEX.yaml`

**Entry Points:** N/A (presentational components, no route)
**Related Files:** `client/src/components/ui/chart.tsx`
**Tests:** `tests/features/chart-recharts-typefix/chart-recharts-typefix.test.ts`
**Spec:** agent/specs/chart-recharts-typefix/spec.md · **Eval:** agent/evaluations/chart-recharts-typefix/eval.md
