...
Create an AI-agent-ready development control system (`/agent/`) to allow future AI agents (Claude Code, Copilot, etc.) to safely navigate, modify, and evaluate this repository without architectural violations.
...
- `/agent/README.md` (System overview & directory map)
- `/agent/SDD_CONTROL.md` (Enforcement rules for Specification-Driven Development)
- `/agent/CHANGE_POLICY.md` (Strict rules on what AI can/cannot touch)

### (B) ARCHITECTURE UNDERSTANDING LAYER

- `/agent/architecture/OVERVIEW.md` (Runtime structure, boundaries, data flow)
- `/agent/architecture/MODULE_MAP.md` (File mappings to logical modules)

### (C) FEATURE-DRIVEN LAYER

- `/agent/features/INDEX.md` (Decomposition of the repo into logical units)
- `/agent/features/<primary-feature>.md` (Detailed feature file: Scope, Risks, Safety Level)

### (D) SPECIFICATION LAYER (SDD)

- `/agent/specs/TEMPLATE.md` (Standardized spec format)
- `/agent/specs/<primary-feature>/spec.md` (One complete, precise example spec)

### (E) EVALUATION LAYER (EDD)

- `/agent/evaluations/TEMPLATE.md` (Standardized evaluation format)
- `/agent/evaluations/<primary-feature>/eval.md` (One complete example eval report)

### (F) AI AGENT OPERATING SYSTEM

- `/agent/AGENT_GUIDE.md` (Mandatory step-by-step reading order for agents; "No spec = no change" enforcement)
