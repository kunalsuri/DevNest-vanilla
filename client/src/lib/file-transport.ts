...
export class FileTransport implements LogTransport {
  ...
  private async initializeFileSystem(): Promise<void> {
    try {
      ...
      if (typeof window === "undefined" && typeof require !== "undefined") {
        ...
        console.log(
          `[FileTransport] Browser mode - logs stored in localStorage with prefix: devnest_logs_`,
        );
      }
    } catch (error) {
  ...
  private async writeToBrowserStorage(
    fileName: string,
    logLine: string,
  ): Promise<void> {
    try {
  ...
  private async rotateFile(fileName: string): Promise<void> {
    ...
    try {
      ...
      for (
        let i = this.config.maxFiles + 1;
        i <= this.config.maxFiles + 5;
        i++
      ) {
        const oldFile = path.join(dirName, `${baseName}.${i}${fileExt}`);
        try {
      ...
      console.error(
        `[FileTransport] Failed to rotate log file ${fileName}:`,
        error,
      );
    }
  }
  ...
  async readLogFile(
    ...
    try {
      ...
      console.error(
        `[FileTransport] Failed to read log file ${fileName}:`,
        error,
      );
      return [];
    }
